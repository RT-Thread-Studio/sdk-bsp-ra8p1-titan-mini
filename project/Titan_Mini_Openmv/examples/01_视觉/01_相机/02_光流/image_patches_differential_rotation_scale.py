# Titan Mini OpenMV example: 分块差分旋转与缩放光流
# Source: ThirdParty/openmv/scripts/examples/01-Camera/02-Optical-Flow/image-patches-differential_rotation_scale.py
# Adaptation: 自定义传感器帧尺寸改为 QVGA 加 64×64/128×128 裁剪，并按裁剪尺寸创建参考图与遍历光流分块。
...
csi0.window((128, 128))  # MIPI capture crop; power-of-two FFT input.
csi0.snapshot(time=2000)  # Wait for settings take effect.
clock = time.clock()  # Create a clock object to track the FPS.

# Create a second frame buffer on the heap.
extra_fb = image.Image(csi0.window()[2], csi0.window()[3], csi0.pixformat())
...
while True:
    ...
    for y in range(0, csi0.window()[3], BLOCK_H):
        for x in range(0, csi0.window()[2], BLOCK_W):
