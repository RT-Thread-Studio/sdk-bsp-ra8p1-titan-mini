...
#line 1 "rt-thread/components/dfs/dfs_v1/filesystems/elmfat/dfs_elm.c"
#include <rtthread.h>
...
/* A partition scan can mask an earlier disk error as NO_FILESYSTEM. */
static rt_bool_t titan_elm_probe_active[FF_VOLUMES];
static rt_bool_t titan_elm_probe_read_failed[FF_VOLUMES];
...
int dfs_elm_close(struct dfs_file *file)
{
    ...
    RT_ASSERT(file->vnode->ref_count > 0);
    /* dfs_file_close already handles dup() references to this descriptor.
     * A different open of the same vnode must not suppress this FIL's close.
     */
    result = FR_OK;
    if (file->vnode->type == FT_DIRECTORY)
    {
...
DRESULT disk_read(BYTE drv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    if (result != count && titan_elm_probe_active[drv])
    {
        titan_elm_probe_read_failed[drv] = RT_TRUE;
    }
    if (result == count)
    {
        return RES_OK;
...
DRESULT disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
    ...
    {
#if defined(BSP_OPENMV_STORAGE_SD)
        /* Generic MMCSD BLK_SYNC is a no-op; wait for the raw SD card itself. */
        if (!rt_strcmp(device->parent.name, "sd") && titan_sdhi_sync() != RT_EOK)
        {
            return RES_ERROR;
        }
#endif
...
/* Project-owned guarded probe/format; uses this view's disk[] mapping. */
#include "../storage/titan_flash_fs.inc"
