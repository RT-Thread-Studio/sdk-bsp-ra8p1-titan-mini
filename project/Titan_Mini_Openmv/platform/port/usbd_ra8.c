...
#include "../drivers/ra8_usb_setup.inc"
...
static void usbd_reset(uint8_t rhport) {
  ...
  // Queued SETUP entries still own the pending count across this reset.
}
  ...
  for (unsigned epr = 0;; epr++) {
    ...
    switch (event.event_id) {
      ...
      case DCD_EVENT_XFER_COMPLETE: {
        ...
        // A later SETUP may already be queued while this older data
        // completion waits. Do not continue it against new hardware state.
        if (epnum == 0 && ra8_core_ep0_completion_obsolete(ep_addr, event.xfer_complete.len)) {
          break;
        }
