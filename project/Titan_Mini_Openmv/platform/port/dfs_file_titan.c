/* Project-maintained platform view; RT-Thread source is unchanged. */
#line 1 "rt-thread/components/dfs/dfs_v1/src/dfs_file.c"
/*
 ...
 */
int titan_dfs_file_open_impl(struct dfs_file *fd, const char *path, int flags)
{
    ...
    if (vnode)
    {
        ...
        fd->pos   = 0;
        fd->vnode = vnode;
        /* Keep the manager lock through fops->open and its rollback, as for
         * a newly allocated vnode. The common exit releases it exactly once.
         */
    ...
    {
        vnode->ref_count--;
        /* POSIX open releases the failed descriptor next. This reference was
         * already dropped here; do not let fd_release drop it a second time.
         */
        fd->vnode = NULL;
 ...
 * @return 0 on successful, -1 on failed.
 */
int titan_dfs_file_close_impl(struct dfs_file *fd)
{
    struct dfs_vnode *vnode = NULL;
    int result = 0;
 ...
 * @return 0 on successful, -1 on failed.
 */
int titan_dfs_file_unlink_impl(const char *path)
{
    int result;
    char *fullpath;
    struct dfs_filesystem *fs;
 ...
 * @return 0 on successful, -1 on failed.
 */
int titan_dfs_file_stat_impl(const char *path, struct stat *buf)
{
    int result;
    char *fullpath;
    struct dfs_filesystem *fs;
 ...
 * @return 0 on successful, -1 on failed.
 */
int titan_dfs_file_rename_impl(const char *oldpath, const char *newpath)
{
    int result = RT_EOK;
    ...
    /* With no SD mounted, both lookups may be NULL. They are not a shared
     * filesystem: report the missing path without dereferencing oldfs. */
