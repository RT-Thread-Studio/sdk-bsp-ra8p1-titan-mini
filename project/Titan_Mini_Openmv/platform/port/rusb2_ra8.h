/* Project-maintained RA8P1 platform view; upstream is unchanged. */
/*
...
static inline void rusb2_utmi_phy_powerup(rusb2_reg_t* rusb) {
  // RA8P1 UM 38.2.17: HSEB reset is unspecified; HS requires zero.
  // Reserved fields must be written as zero.
  const uint16_t writable = RUSB2_PHYSET_REPSTART_Msk | RUSB2_PHYSET_REPSEL_Msk |
                            RUSB2_PHYSET_CLKSEL_Msk | RUSB2_PHYSET_CDPEN_Msk |
                            RUSB2_PHYSET_PLLRESET_Msk;
  uint16_t physet = (rusb->PHYSET & writable) | RUSB2_PHYSET_DIRPD_Msk;
  rusb->PHYSET = physet;
