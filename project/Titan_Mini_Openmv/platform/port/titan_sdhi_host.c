/* Project-maintained SDK view; hardware sequence matches Titan_Mini_driver_all. */
/*
...
rt_err_t command_send(sdhi_instance_ctrl_t *p_ctrl, struct rt_mmcsd_cmd *cmd)
{
    ...
    while (true)
    {
        ...
        {
            cmd->err = -RT_ERROR;
            if ((event.word & HW_SDHI_ERR_CRCE) && (((cmd->flags & RESP_MASK) == RESP_R3) || ((cmd->flags & RESP_MASK) == RESP_R4)))
            {
            ...
            if (data && (event.word & HW_SDHI_ERR_DTIMEOUT))
            {
...
#include "../storage/titan_sdhi_request.inc"
...
void sdhi0_callback(sdmmc_callback_args_t *p_args)
{
    if (p_args->event & SDMMC_EVENT_CARD_REMOVED) {
        if (host0 && host0->card) { sd_media_invalidated = RT_TRUE; }
    }
    /* enter interrupt */
    rt_interrupt_enter();
    ...
    if ((SDMMC_EVENT_ERASE_COMPLETE | SDMMC_EVENT_TRANSFER_COMPLETE) & p_args->event)
    {
       /* Process All SDIO Interrupt Sources */
        if (host0) { rthw_sdio_irq_process(host0); }
    }
...
struct rt_mmcsd_host *sdio_host_create(struct ra_sdhi *sdhi_des, rt_uint8_t cache_buf[SDIO_BUFF_SIZE])
{
    ...
    /* Project mount starts one serialized probe after USB is ready. */
...
{
#ifdef BSP_USING_SDHI0
    struct ra_sdhi sdhi0 = {0};
    sdhi0.instance = &g_sdmmc0;
    sdhi0.bus_width = BSP_SDHI0_BUS_WIDTH; /* Set bus width from Kconfig */
    fsp_err_t result = sdhi0.instance->p_api->open(sdhi0.instance->p_ctrl, sdhi0.instance->p_cfg);
    if (result != FSP_SUCCESS) { rt_kprintf("[titan.sd] SDHI0 open failed: %d\n", result); return -RT_ERROR; }
