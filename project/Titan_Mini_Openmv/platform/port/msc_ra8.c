...
void ra8_msc_complete_now(int32_t bytes_io) {
...
#if defined(BSP_OPENMV_STORAGE_FLASH)
  TU_VERIFY_STATIC(TITAN_FLASH_DISK_SECTOR_SIZE == 512U, "Review MSC packet alignment for this medium");
  TU_VERIFY_STATIC(TITAN_FLASH_ERASE_SIZE % TITAN_FLASH_DISK_SECTOR_SIZE == 0U, "Erase size must contain whole sectors");
  TU_VERIFY_STATIC(TITAN_FLASH_STORAGE_OFFSET % TITAN_FLASH_DISK_SECTOR_SIZE == 0U, "Storage must start on a sector boundary");
  TU_VERIFY_STATIC(CFG_TUD_MSC_EP_BUFSIZE % TITAN_FLASH_DISK_SECTOR_SIZE == 0U, "MSC buffer must contain whole sectors");
  if (rdwr10_get_blocksize(p_cbw) == TITAN_FLASH_DISK_SECTOR_SIZE &&
      p_cbw->total_bytes % TITAN_FLASH_DISK_SECTOR_SIZE == 0U &&
      p_msc->xferred_len % TITAN_FLASH_DISK_SECTOR_SIZE == 0U) {
    uint64_t const address = (uint64_t)TITAN_FLASH_STORAGE_OFFSET +
      (uint64_t)rdwr10_get_lba(p_cbw->command) * TITAN_FLASH_DISK_SECTOR_SIZE + p_msc->xferred_len;
    uint32_t const to_boundary = TITAN_FLASH_ERASE_SIZE - (uint32_t)(address % TITAN_FLASH_ERASE_SIZE);
    nbytes = (uint16_t)tu_min32(nbytes, to_boundary);
  }
#endif
