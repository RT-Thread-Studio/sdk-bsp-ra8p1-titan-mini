...
#line 1 "rt-thread/components/dfs/dfs_v1/src/dfs_fs.c"
/*
 ...
 */
int titan_dfs_mount_impl(const char   *device_name,
              const char   *path,
 ...
 * @return 0 on successful or -1 on failed.
 */
int titan_dfs_unmount_impl(const char *specialfile)
{
    char *fullpath;
    struct dfs_filesystem *iter;
    struct dfs_filesystem *fs = NULL;
 ...
 */
int titan_dfs_mkfs_impl(const char *fs_name, const char *device_name)
{
    int index;
 ...
 */
int titan_dfs_statfs_impl(const char *path, struct statfs *buffer)
{
